...
raw_audio = ROOT / "public/audio/ep012-voice-cartesia-spliced-v01.wav"
processed_audio = ROOT / "public/audio/ep012-voice-cartesia-processed-v01.wav"
trim_json = ROOT / "creatorflow/ep012-voice-trim-v01.json"
words_path = ROOT / "creatorflow/ep012-words-v01.json"
captions_path = ROOT / "creatorflow/ep012-captions-v01.json"
voiceover_txt = ROOT / "public/audio/ep012-voiceover-v01.txt"

# 1. Trim leading and trailing silence AND apply atempo=1.1 (1.1x speedup)
cmd_trim = [
    f"{ffmpeg_dir}/ffmpeg", "-y", "-i", str(raw_audio),
    "-af", "atempo=1.1,silenceremove=start_periods=1:start_duration=0.08:start_threshold=-40dB,areverse,silenceremove=start_periods=1:start_duration=0.08:start_threshold=-40dB,areverse",
    str(processed_audio)
]
...
trim_meta = {
    "method": "atempo-1.1-silenceremove",
    "speed": 1.1,
    "thresholdDb": -40,
    "minimumSilenceMs": 80,
...
print(f"Trimmed & sped up (1.1x) audio: {raw_ms}ms -> {proc_ms}ms (saved to {processed_audio.name})")
...
for i in range(1, len(final_words)):
    if final_words[i]["startMs"] <= final_words[i-1]["startMs"]:
        final_words[i]["startMs"] = final_words[i-1]["startMs"] + 25
    if final_words[i]["endMs"] <= final_words[i]["startMs"]:
        final_words[i]["endMs"] = final_words[i]["startMs"] + 50

words_payload = {
    "source": "cartesia-sonic-3.5-spliced-1.1x",
