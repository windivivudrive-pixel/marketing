...
const words = wordsData as Caption[];
const normalize = (value: string) => value.toLocaleLowerCase("vi").replace(/[^\p{L}\p{N}]+/gu, "");

export const Captions = ({tokens, top = 1110}: {tokens: StyleTokens; top?: number}) => {
  ...
  const captionWords = words.filter((word) => word.endMs > caption.startMs && word.startMs < caption.endMs);
  let searchFrom = 0;
  const tokensWithTiming = caption.text.split(/(\s+)/).map((text) => {
    if (/^\s+$/.test(text)) return {text, word: undefined};
    const key = normalize(text);
    const relative = captionWords.slice(searchFrom).findIndex((word) => normalize(word.text) === key);
  ...
  return (
    <div style={{position: "absolute", top, left: 90, right: 150, textAlign: "center", zIndex: 30}}>
      <div style={{display: "inline-block", maxWidth: 820, padding: "16px 30px", border: "none", borderRadius: 16, background: tokens.surface, boxShadow: `0 8px 24px ${tokens.shadow}`, color: tokens.text, fontFamily: tokens.bodyFont, fontSize: 38, lineHeight: 1.24, fontWeight: 800, whiteSpace: "pre-wrap"}}>
        {tokensWithTiming.map(({text, word}, index) => {
          if (!word) return <span key={`${index}-${text}`}>{text}</span>;
          const active = now >= word.startMs && now < word.endMs;
          const wordFrame = frame - Math.round((word.startMs / 1000) * fps);
          const bounce = active ? spring({frame: Math.max(0, wordFrame), fps, config: {damping: 13, stiffness: 260, mass: 0.55}}) : 0;
